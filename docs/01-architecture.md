...
本文回答:**多用户社区形态下,整体架构能否保持低耦合?职责分层是否清晰?**
结论先行,见第 11 节。
...
因此整体架构没有被社区化推翻,只是多了"归属/可见性"和"社交层"。
...
> 多用户**地基**(认证、用户体系、内容归属、多租户隔离)在 M0 就建;社区**功能**(评论、讨论、关注、动态流)分期到 M4 才建。

原因:地基后补等于重写(给一个单用户系统塞进多租户极其痛苦);而社区功能是挂在地基上的,后加成本低。且别在没人的平台上先建讨论区——社区功能要等"先小范围"那批朋友就位时再上。
...
- **web** 是**一个** Next.js 应用,不拆成多个。访客、登录用户、管理员是同一应用内的不同角色,用 Next.js route group + 路由层鉴权区分。多用户不等于多应用。
...
- **开放公开注册**:`identity` 从 M0 就是多用户的,开放注册只是放开注册入口 + 启用 `moderation` 的防滥用功能,不动核心架构。
...
**多用户社区没有破坏低耦合**,因为它是"叠加维度 + 叠加一层",不是"重写"。
